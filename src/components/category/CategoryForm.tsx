...
import ProductUploadWrapper from "../productUpload/ProductUploadWrapper"
            ...
            <div className="flex lg:flex-row items-start justify-start lg:gap-8 md:gap-6 gap-4 flex-col xl:py-8 lg:py-6 py-4 lg:px-7 px-3  bg-white w-full rounded-2xl shadow-sm">
                ...
                >
                    <FormMessage loading={isLoading} errorMessage={errorMessage} />
                        <ProductUploadWrapper title="Product Category Details">
                            ...
                            <div className="flex flex-col">
                                <select disabled={isLoading} value={formData?.markupType} name="markupType" onChange={(e) => setFormData({...formData, markupType: e.target.value})} className="block xl:py-6 lg:py-4 py-3 xl:px-7 lg:px-[15px] px-3 w-full placeholder:text-dark_500 text-dark_500 bg-[#fdfdfd] rounded-lg border border-[#cec1c1] border-opacity-80 font-inter xl:text-lg lg:text-base text-sm xl:placeholder:text-lg lg:placeholder:text-base placeholder:text-sm focus:outline-none transition duration-300 focus:border-primary_100 focus:ring-[0.5px] focus:ring-primary_100 outline-none ">
                                    <option value="">Select Markup Type</option>
                                    <option value="percentage">Percentage</option>
                                    <option value="value">Value</option>
                                </select>
                                {formDataError?.markupType && <span className="text-red-500 text-sm mt-1">{formDataError?.markupType}</span>}
                            </div>
                            ...
                            {formData?.markupType && (
                                <div className="flex flex-col">
                                    <input
                                        ...
                                        name="markupValue"
                                        className="block xl:py-6 lg:py-4 py-3 xl:px-7 lg:px-[15px] px-3 w-full placeholder:text-dark_500 text-dark_500 bg-[#fdfdfd] rounded-lg border border-[#cec1c1] border-opacity-80 font-inter xl:text-lg lg:text-base text-sm xl:placeholder:text-lg lg:placeholder:text-base placeholder:text-sm focus:outline-none transition duration-300 focus:border-primary_100 focus:ring-[0.5px] focus:ring-primary_100 outline-none "
                                        placeholder={`Type In Markup by ${formData?.markupType}`}
                                        disabled={isLoading}
                                        defaultValue={formData?.markupValue}
                                        onChange={(e) => setFormData({...formData, markupValue: e.target.value})}
                                    />
                                    {formDataError?.markupValue && <span className="text-red-500 text-sm mt-1">{formDataError?.markupValue}</span>}
                                </div>
                            )}  
